...
export function areCompatible(card: Card, lastCardPlayed: Card) {
  return card.rank ==='Q' || lastCardPlayed.rank ==='Q' || card.rank === lastCardPlayed.rank || card.suit === lastCardPlayed.suit
}
...
export function playersWithOneOrFewerCard(gameState: GameState): string[] {
  const playerCardCounts = computePlayerCardCounts(gameState)
  return gameState.playerNames.filter((_, index) => playerCardCounts[index] <= 1)
}
