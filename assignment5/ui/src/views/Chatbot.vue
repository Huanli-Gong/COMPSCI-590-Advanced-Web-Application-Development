      ...
      <transition-group name="list">
        ...
        >
          ...
          <div>
            ...
            <span v-for="word in m.content.match(/[\w']+|[^\w\s]+/g)" style="display: inline-block">
                <template v-if="isIngredient(word)">
                  <b-badge @click="addIngredient(word)" style="cursor: pointer;">{{ word }}</b-badge>
                </template>
                <template v-else>
                  {{ word }}
                </template>
...
import { Ingredient } from "../../../server/data"
...
const emit = defineEmits<{
  (e: 'addIngredient', ingredient: Ingredient): void
}>()

function isIngredient(word: string) {
    return (props.possibleIngredients ?? []).map(ingredient => ingredient.name.toLowerCase()).includes(word.toLowerCase())
}

function addIngredient(ingredientName: string) {
    const ingredient = (props.possibleIngredients ?? []).find(i => i.name.toLowerCase() === ingredientName.toLowerCase())
    if (ingredient){
