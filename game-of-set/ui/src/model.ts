...
export class Table {
    ...
    replaceUsedCards(deck: Deck, usedCards: Card[], tableSize: number = 12): void {
...
export function determineWinner(state: GameState) {
    ...
    let scores: number[] = []
    state.players.forEach((player, _) => {
...
function addCards(state: GameState) {
    if (!state.deck.isEmpty()) {
        for (let i = 0; i < 3; i++) {
            const addedCard = state.deck.cards.shift()
            if (addedCard) {
                state.table.tableAdd(addedCard)
            }
        }
    }
}
...
export function printState({ deck, table, players, currentTurnPlayerIndex, phase, playCount }: GameState) {
    ...
    console.log("Silence warning: ", deck.isEmpty(), table.isEmpty(), phase, playCount)
}